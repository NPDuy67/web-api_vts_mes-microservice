...
public class WorkOrdersController : ApiControllerBase
{
    ...
    {
        var command = new CreateWorkOrderCommand(manufacturingOrderId, workOrder.WorkOrderId, workOrder.DueDate, workOrder.StartTime, workOrder.EndTime, workOrder.WorkOrderStatus, workOrder.PrerequisiteOperations, workOrder.WorkCenter);
    ...
    [HttpGet]
    [Route("DownloadReport/{manufacturingOrderId}/{workOrderId}")]
    public async Task<IActionResult> DownloadReport([FromRoute] string manufacturingOrderId, [FromRoute] string workOrderId)
    {
        var query = new DownloadReportQuery(manufacturingOrderId, workOrderId);
