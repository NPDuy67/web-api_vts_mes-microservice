...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
    ...
    public void AddOperation(string operationId, string name, TimeSpan duration, List<string> prerequisiteOperationId)
    {
        List<Operation> prerequisiteOperations = prerequisiteOperationId.ConvertAll(id => GetOperationWithId(id));

        var operation = new Operation(operationId, name, duration, prerequisiteOperations, Id);
    ...
    public void UpdateOperation(string operationId, string name, TimeSpan duration, List<string> prerequisiteOperationId)
    {
        ...
        try
        {
            operation.Update(name, duration, prerequisiteOperations);
        }
        catch (Exception ex)
        {
            throw new DomainException($"MaterialDefinition with id {ResourceId} throw an exception. See inner exception for details.", ex);
